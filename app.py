...
# --- 2. Get Backend URL from Secrets ---
try:
...
with tab1:
    ...
    if st.button("Predict Subscription Probability", type="primary"):
        # The key names here must match your CustomerInput schema in api/schemas.py
        payload = {
        ...
        with st.spinner("Analyzing Lead..."):
            try:
                # MATCHING YOUR BACKEND: @router.post("/predict")
                response = requests.post(f"{BACKEND_URL}/predict", json=payload, timeout=30)
                if response.status_code == 200:
                    res = response.json()
                    # MATCHING YOUR BACKEND KEY: "probability_of_subscription"
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Consulting knowledge base..."):
                try:
                    # MATCHING YOUR BACKEND: @router.post("/research")
                    resp = requests.post(f"{BACKEND_URL}/research", json={"query": prompt}, timeout=90)
                    if resp.status_code == 200:
                        # MATCHING YOUR BACKEND KEY: "answer"
